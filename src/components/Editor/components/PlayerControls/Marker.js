...
import { LeftBracket, RightBracket } from 'assets/icons';
const MarkerPoint = ({
  width,
  totalDuration,
  from,
  ...
  return (
    <div className="timeline" style={{ width: `${width}%` }}>
      ...
      >
        {position == 'start' ? (
          <LeftBracket onClick={e => e.stopPropagation()} className="pin" src={img} />
        ) : position == 'end' ? (
          <RightBracket onClick={e => e.stopPropagation()} className="pin" src={img} />
        ) : (
